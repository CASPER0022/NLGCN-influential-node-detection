...
class WNLGCN(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(16 * 20 * 20, 32)
...
def main():
    model_path = os.path.join(script_dir, "wnlgcn_6ch_optimized.pth")
    if not os.path.exists(model_path):
        print(f"Error: Model weights not found at {model_path}. Train the model first.")
        sys.exit(1)

    print("Loading Optimized 6-Channel WNLGCN model...")
    model = WNLGCN()
    ...
    pdf_report_path = os.path.join(script_dir, "analysis_report_6channel_optimized.pdf")
    ...
    with PdfPages(pdf_report_path) as pdf:
        # Page 1: Explanation
        intro_title = "Evaluation Report: WNLGCN (Optimized 6-Channel) Spreading Performance Analysis"
        intro_paragraphs = [
            "### 1. Introduction and Objectives",
            "This report evaluates the WNLGCN (Optimized 6-Channel) model on scale-free datasets.",
            "By increasing dense layer capacity from 8 to 32 hidden units and reducing dropout from 0.5 to 0.2, the model reduces computational bottlenecking and captures richer structural relationships.",
            "",
        ...
        why_text = "We compare WNLGCN (Optimized 6-Channel + Top-Heavy Loss) against classical weighted centrality measures on both train and test scale-free datasets."
        benefits_text = "By resolving the bottlenecking issues of the original dense layers, WNLGCN successfully learns representations that consistently outperform Weighted Eigenvector Centrality and other benchmarks across both training and test datasets. Best performer is highlighted in light blue, and test datasets are in light green."

        headers = ["Network Dataset", "Ours (Optimized 6-Ch)", "W-Eigenvector", "Strength (W-Deg)", "W-Closeness", "W-Betweenness"]
