...
datasets_dir = os.path.abspath(os.path.join(script_dir, "..", "..", "Datasets", "scalefree networks", "train"))
results_dir = os.path.abspath(os.path.join(script_dir, "..", "results"))
os.makedirs(results_dir, exist_ok=True)

# 18 Scale-Free Training networks
training_datasets = [
    "Budapest.txt",
    "C_elegans.txt",
    "E.coli.edge",
...
def process_dataset(filepath, filename, L=40, precalculated_y=None):
    ...
    # NO LOG NORMALIZATION - raw values
...
def main():
    ...
    for filename in training_datasets:
        ...
        # Save cache files with suffix raw_local_norm_X to avoid conflicts
        ...
        try:
            if os.path.exists(cache_x_path) and os.path.exists(cache_y_path):
                print(f"Loading cached raw local-normalized features and SIR labels for {filename}...")
                X = np.load(cache_x_path)
                y = np.load(cache_y_path)
            elif os.path.exists(cache_y_path):
                print(f"Loading cached SIR labels for {filename} and regenerating raw local-normalized features...")
                y = np.load(cache_y_path)
                X, _ = process_dataset(filepath, filename, precalculated_y=y)
                np.save(cache_x_path, X)
                print(f"Regenerated raw local-normalized features saved to {cache_x_path}.")
            else:
                X, y = process_dataset(filepath, filename)
                np.save(cache_x_path, X)
                np.save(cache_y_path, y)
                print(f"Cached raw local-normalized features and SIR labels for {filename} saved to {results_dir}.")
            all_X.append(X)
            all_y.append(y)
