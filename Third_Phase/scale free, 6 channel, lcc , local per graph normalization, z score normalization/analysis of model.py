...
# Keep the cache local to this scale-free folder
...
def get_all_dataset_files(root_dir):
    ...
    for dirpath, _, filenames in os.walk(root_dir):
        for f in filenames:
            ...
            if ext in valid_exts or f.startswith("out.") or f in target_datasets:
...
# ---- Render Matplotlib Table with Highlighted Test Rows, Title, and Explanation ----
def add_table_page(pdf, title, headers, data, row_statuses, best_cols_per_row, why_text, benefits_text):
    ...
    # 3. Render 'Why It Benefits Us' explanation block
    y_pos -= 0.01
    ...
    for row_idx in range(1, row_count + 1):
        ...
        # Test rows are light green, train rows are white/light gray
        ...
        for col_idx in range(len(headers)):
            ...
            if col_idx == 0:
                ...
                # Highlight the maximum/best correlation cell in a premium blue color (#D9E1F2 or #B4C6E7)
...
def main():
    ...
    pdf_report_path = os.path.join(script_dir, "analysis_report.pdf")
    print(f"\nGenerating simplified PDF Analysis Report at {pdf_report_path}...")
    
    with PdfPages(pdf_report_path) as pdf:
        
        # Page 1: Introduction & Methodology Explanations
        intro_title = "Evaluation Report: NLGCN Model Spreading Performance Analysis"
        intro_paragraphs = [
        ...
        for pct in [20]:
            ...
            unw_why = (
                f"We compare the prediction correlation of our NLGCN model against classical unweighted centrality heuristics "
            ...
            unw_headers = ["Network Dataset", "Ours (NLGCN)", "Degree", "Closeness", "Betweenness", "Eigenvector"]
            unw_rows = []
            unw_statuses = []
            unw_best_cols = []
            ...
            for name, status, pct_data in sir_unweighted_results:
                ...
                # Find column index of the maximum numeric correlation value (indices 0 to 4 map to cols 1 to 5)
            ...
            w_headers = ["Network Dataset", "Ours (NLGCN)", "Strength (W-Deg)", "W-Closeness", "W-Betweenness", "W-Eigenvector"]
            w_rows = []
            w_statuses = []
            w_best_cols = []
    ...
    print("\nPDF 'analysis_report.pdf' generated successfully!")
