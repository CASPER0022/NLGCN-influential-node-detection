...
def top_heavy_pairwise_ranking_loss(pred, y, is_top, margin=0.4, top_weight=4.0):
    pred = pred.view(-1)
    y = y.view(-1)
...
def main():
    ...
    # 2. Train Model with AdamW and Cosine Annealing
    ...
    print("Starting Deep 6-Channel WNLGCN Training (AdamW + Cosine LR)...")
    ...
    # Save model weights
    model_save_path = os.path.join(script_dir, "wnlgcn_6ch_deep.pth")
    ...
    # 3. Evaluation
    print("\n" + "="*100)
    print(f"{'Network Dataset':<30} | {'Ours (Deep 6-Ch)':<20} | {'W-Eigenvector':<15} | {'Strength (W-Deg)':<15} | {'W-Closeness':<15}")
    print("="*100)
